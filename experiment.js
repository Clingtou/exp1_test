...
function roseChartHtml(condition, options = {}) {
  ...
  if (condition.position_condition === "top" || condition.position_condition === "bottom") {
    ...
    const sideY = cy - 24;
    labelHtml = `
      ${calloutTextHtml(cx - left.radius - edgeGap, sideY, left.label, left.amount, "end")}
      ${calloutTextHtml(cx + right.radius + edgeGap, sideY, right.label, right.amount, "start")}
    `;
  }
  ...
  return {
    ...
    stimulus: shellHtml(`
      ...
      <ul>
        ...
        <li>If you <span class="doc-red">reject</span>, this means that the two of you have not reached an agreement, and neither you nor the proposer will receive any part of the 100-cent bonus.</li>
      </ul>
      ...
      <p>In this study, <span class="doc-red">${BONUS_DRAW_PERCENT}%</span> of receivers will be randomly selected to have their decisions implemented for real payment. If you are selected, we will implement your decision exactly as specified above: the 100-cent bonus will be allocated between you and the matched proposer according to your choice, and the outcome will be paid as a Prolific bonus. Bonus payments will be processed within two months after data collection is complete.</p>
...
async function buildAndRunExperiment() {
  ...
  timeline.push({
    ...
    message: `<div class="fullscreen-message">
      ...
      <p>This study is a decision-making study conducted by Zhejiang University. The purpose of this study is to explore how people evaluate options and make decisions in social and economic situations.</p>
      <p>Your participation is voluntary. You may stop participating at any time before submitting your response. If you choose not to continue, please close this browser window or return the study on Prolific.</p>
      <p>All personal data collected in this study will be used for research purposes only. The data will not be used for any commercial purpose. Your responses will be analyzed anonymously and reported only in aggregate form.</p>
      <p>This study must be completed on a desktop or laptop computer. Please enter fullscreen mode to begin. If you exit fullscreen before the study ends, the study will stop automatically.</p>
