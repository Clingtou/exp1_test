...
function roseChartHtml(condition, options = {}) {
  ...
  if (condition.position_condition === "top" || condition.position_condition === "bottom") {
    ...
    cy = fixedGroupBottom - lineGap - amountTextHeight - edgeGap + labelMidY - shapeBounds.maxY;
    const upperY = cy + shapeBounds.minY - edgeGap - lineGap - amountMidY;
    const lowerY = cy + shapeBounds.maxY + edgeGap - labelMidY;
    labelHtml = `
    ...
    const sideY = cy - 24;
    labelHtml = `
      ${calloutTextHtml(cx + shapeBounds.minX - edgeGap, sideY, left.label, left.amount)}
      ${calloutTextHtml(cx + shapeBounds.maxX + edgeGap, sideY, right.label, right.amount)}
    `;
  }
